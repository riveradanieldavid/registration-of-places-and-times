                ...
                {data.map(item => {
                    ...
                    return (
                        <React.Fragment key={item.id}>
                            ...
                            {/* CONDUCTOR */}
                            <div
                                className={`expand-info grid-item ${item.date.trim() === today ? 'highlight' : ''
                                    } ${expandedRowId === item.id ? 'selected-row' : ''}`}
                            >
                                {item.servant}
                            </div>
                            ...
                            <div
                                ...
                                ${expandedRowId === item.id ? 'selected-row' : ''}
                                ${isEditing && editingItemId === item.id ? 'grid-edition-actions' : ''}
                                `}
                            >
                                {/* BOTON EDITAR */}
                                <button
                                    className={`'editionButton' ${isEditing && editingItemId === item.id ? 'editingButton' : ''}`}
                                    onClick={(e) => {
                                        e.stopPropagation()
                                        handleEdit(item.id)
                                    }}
                                ...
                                >
                                    {isEditing && editingItemId === item.id ? 'SE ESTA EDITANDO ESTE ITEM' : 'EDITAR'}
                                </button>
                                {/* MESSAGE ITEM ACTUALIZADO */}
                                {!isEditing &&
                                    <button
                                        ref={formRef}
                                        className={`message ${message && editingItemId === item.id ? 'show' : 'hidden'}`}>
                                        {message}
                                    </button>
                                }
                                {/* BOTON ELIMINAR */}
                            ...
                            {expandedRowId === item.id && (
                                ...
                                >
                                    {/* CONDUCTOR */}
                                    <div
                                        className={`expanded-info-header grid-header ${item.date.trim() === today ? 'highlight' : ''
                                            }`}
                                    >
                                        Conductor
                                    </div>
